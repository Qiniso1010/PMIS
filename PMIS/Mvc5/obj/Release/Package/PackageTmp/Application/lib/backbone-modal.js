...
(function(){
  ...
  Backbone.ModalView = Backbone.View.extend({
    ...
    template: _.template([
      ...
      '      <a type="button" class="close" aria-hidden="true"><i class="lnr lnr-cross size-48"></i></a>',
      '      <%=title%>',
      '    </div>',
    ...
    buttonTemplate: _.template('<a href="<%=href%>" class="btn <%=className%> btn-custom r-2"><%=label%></a>'),
    buttonDefaults: {
      className: "",
      href: "javascript:;",
      label: "",
    ...
    defaults: {
      ...
      body: "",
      buttons: [{
        className: "btn bg-blue btn-custom",
        href: "javascript:;",
        label: "Close",
        close: true
      }],
